...
function parse3MFModel(xml: string): { positions: number[] } {
  ...
  logger.info(`   📝 XML length: ${xml.length} characters`);
  logger.info(`   📝 XML preview: ${xml.substring(0, 500)}...`);
  ...
  logger.info(`   ✅ Found ${vertices.length} vertices`);
  
  if (vertices.length > 0) {
    logger.info(`   📊 First vertex: x=${vertices[0].x}, y=${vertices[0].y}, z=${vertices[0].z}`);
  }
  ...
  while ((match = triangleRegex.exec(xml)) !== null) {
    ...
    if (v1 && v2 && v3) {
      ...
      logger.info(`   ⚠️ Invalid vertex indices: v1=${idx1}, v2=${idx2}, v3=${idx3}`);
    }
  }
  
  logger.info(`   ✅ Found ${triangleCount} triangles`);
  
  if (triangleCount === 0 && vertices.length > 0) {
    logger.info(`   ⚠️ Warning: Found vertices but no triangles!`);
    logger.info(`   📝 Searching for triangle patterns in XML...`);
    // Log sample of XML around <mesh> tags
    const meshMatch = xml.match(/<mesh[\s\S]{0,1000}/i);
    if (meshMatch) {
      logger.info(`   📝 Mesh section: ${meshMatch[0].substring(0, 500)}...`);
    }
  }
...
function load3MF(filePath: string): { positions: number[] } {
  logger.info('   📦 Opening 3MF ZIP archive...');
  const zip = new AdmZip(filePath);
  const zipEntries = zip.getEntries();
  
  logger.info(`   📦 ZIP contains ${zipEntries.length} files`);
  logger.info(`   📦 Available entries: ${zipEntries.map((e: any) => e.entryName).join(', ')}`);
  
  // Find ALL .model files (main model + object files)
  const modelFiles = zipEntries.filter((entry: any) => entry.entryName.endsWith('.model'));
  logger.info(`   📦 Found ${modelFiles.length} model files: ${modelFiles.map((e: any) => e.entryName).join(', ')}`);
  ...
  for (const modelEntry of modelFiles) {
    logger.info(`   📄 Parsing ${modelEntry.entryName}...`);
    ...
    if (result.positions.length > 0) {
      ...
      logger.info(`   ✅ Added ${result.positions.length / 9} triangles from ${modelEntry.entryName}`);
    }
  }
  
  logger.info(`   ✅ Total combined: ${totalTriangles} triangles, ${allPositions.length} positions`);
...
function parseBinarySTL(buffer: Buffer): { positions: number[] } {
  logger.info('   📝 Trying binary STL parser...');
  ...
  logger.info(`   Triangle count from header: ${triangleCount}`);
  ...
  logger.info(`   ✅ Binary parser found ${triangleCount} triangles`);
  return { positions };
}
...
function parseAsciiSTL(buffer: Buffer): { positions: number[] } {
  logger.info('   📝 Trying ASCII STL parser...');
  ...
  logger.info(`   ✅ ASCII parser found ${vertexCount} vertices (${vertexCount / 3} triangles)`);
  return { positions };
}
...
function loadSTL(filePath: string): { positions: number[] } {
  ...
  try {
    logger.info('   📦 Strategy 1: Using stl-parser library...');
    ...
    if (result && result.facets && result.facets.length > 0) {
      ...
      logger.info(`   ✅ stl-parser found ${result.facets.length} triangles`);
      return { positions };
    }
    
    logger.info('   ⚠️ stl-parser returned empty data, trying fallback...');
  } catch (error: any) {
    logger.info(`   ⚠️ stl-parser failed: ${error.message}`);
  }
  ...
  try {
    ...
    logger.info(`   ⚠️ Binary parser failed: ${error.message}`);
  }
  ...
  try {
    ...
    logger.info(`   ⚠️ ASCII parser failed: ${error.message}`);
  }
...
function detectMaterialCount(filePath: string): number {
  try {
    ...
    if (!modelEntry) {
      logger.info('   ⚠️ No model file found in 3MF, assuming single material');
      return 1;
    }
    ...
    if (baseMaterialsMatch) {
      ...
      if (materialMatches && materialMatches.length > 1) {
        logger.info(`   🎨 Found ${materialMatches.length} materials in <basematerials>`);
    ...
    if (pidMatches) {
      ...
      if (uniquePids.size > 1) {
        logger.info(`   🎨 Found ${uniquePids.size} unique material PIDs`);
    ...
    if (colorMatches && colorMatches.length > 1) {
      logger.info(`   🎨 Found ${colorMatches.length} color definitions`);
      return Math.min(colorMatches.length, 8); // Cap at 8 colors
    }
    
    // Default: single material
    logger.info('   🎨 No multi-material indicators found, assuming single color');
    return 1;
  } catch (error: any) {
    logger.info(`   ⚠️ Could not detect materials: ${error.message}`);
...
function estimatePurgeWaste(fileExt: string, filePath: string): number {
  logger.info(`   🗑️  Estimating purge/tower waste...`);
  
  // STL files: Single material assumed, minimal priming waste
  if (fileExt === '.stl') {
    logger.info(`   🗑️  STL file: single material, adding 15% priming waste`);
    return 1.15;
  }
  ...
  logger.info(`   🗑️  Unknown format, adding 15% waste`);
  return 1.15;
}
...
function computeVolumeAndSurfaceArea(positions: number[]): { 
  ...
  logger.info(`   🔢 Computing volume from ${positions.length} position values...`);
  ...
  if (positions.length % 9 !== 0) {
    logger.info(`   ⚠️ Warning: positions.length (${positions.length}) is not divisible by 9`);
  }
  
  const triangleCount = Math.floor(positions.length / 9);
  logger.info(`   📐 Processing ${triangleCount} triangles...`);
  
  // Sample first few coordinates for debugging
  if (positions.length >= 9) {
    logger.info(`   🔍 First triangle vertices:`);
    logger.info(`      v1: (${positions[0].toFixed(2)}, ${positions[1].toFixed(2)}, ${positions[2].toFixed(2)})`);
    logger.info(`      v2: (${positions[3].toFixed(2)}, ${positions[4].toFixed(2)}, ${positions[5].toFixed(2)})`);
    logger.info(`      v3: (${positions[6].toFixed(2)}, ${positions[7].toFixed(2)}, ${positions[8].toFixed(2)})`);
  }
  ...
  logger.info(`      X: ${minX.toFixed(2)} to ${maxX.toFixed(2)} = ${sizeX.toFixed(2)} mm`);
  logger.info(`      Y: ${minY.toFixed(2)} to ${maxY.toFixed(2)} = ${sizeY.toFixed(2)} mm`);
  logger.info(`      Z: ${minZ.toFixed(2)} to ${maxZ.toFixed(2)} = ${sizeZ.toFixed(2)} mm`);
  logger.info(`      Max dimension: ${maxDimension.toFixed(2)} mm`);
  logger.info(`   📊 Raw signed volume: ${volume.toFixed(2)} mm³`);
  logger.info(`   📊 Absolute volume: ${absVolume.toFixed(2)} mm³`);
  logger.info(`   📊 Surface area: ${surfaceArea.toFixed(2)} mm²`);
  
  // Calculate surface area to volume ratio (key metric for shell factor)
  const saToVolumeRatio = surfaceArea / absVolume;
  logger.info(`   📊 SA/V ratio: ${saToVolumeRatio.toFixed(4)} (higher = more hollow/thin-walled)`);
  ...
  logger.info(`   📊 Bounding box volume: ${boundingBoxVolume.toFixed(2)} mm³`);
  logger.info(`   📊 Mesh fill ratio: ${(volumeRatio * 100).toFixed(1)}%`);
  
  if (absVolume < 1 && triangleCount > 100) {
    logger.info(`   ⚠️ WARNING: Volume suspiciously small (${absVolume.toFixed(4)} mm³) for ${triangleCount} triangles`);
    logger.info(`   ⚠️ This might indicate unit conversion issue or mesh errors`);
  }
...
function estimateSupportPercentage(positions: number[]): number {
  ...
  logger.info(`   🏗️  Overhang analysis: ${(overhangRatio * 100).toFixed(1)}% of triangles need support`);
  logger.info(`   🏗️  Estimated support material: ${supportPercent.toFixed(1)}%`);
...
function calculateDynamicShellFactor(surfaceArea: number, volume: number, meshFillRatio: number): number {
  ...
  logger.info(`   📐 SA/V ratio: ${saVolumeRatio.toFixed(4)} → Base shell: ${baseShellFactor.toFixed(3)}`);
  logger.info(`   📦 Mesh fill: ${fillPercent.toFixed(1)}% → Multiplier: ${fillMultiplier.toFixed(2)}×`);
  logger.info(`   ⚙️ Final shell factor: ${shellFactor.toFixed(3)}`);
...
export async function calculateWeight({
  ...
  try {
    ...
    if (ext === '.stl') {
      logger.info('📦 Loading STL file...');
      geometryData = loadSTL(filePath);
    } else if (ext === '.3mf') {
      logger.info('📦 Loading 3MF file...');
    ...
    logger.info(`   📊 Positions array length: ${geometryData.positions.length}`);
    logger.info(`   📊 Triangle count: ${geometryData.positions.length / 9}`);
    ...
    logger.info(`📐 Mesh volume: ${volumeMm3.toFixed(2)} mm³`);
    
    // Convert to cm³ (1 cm³ = 1000 mm³)
    const volumeCm3 = volumeMm3 / 1000;
    logger.info(`   Volume: ${volumeCm3.toFixed(2)} cm³`);
    
    // Apply scale factor (cube of scale percentage)
    const scaleFactor = scalePercent / 100;
    const scaledVolumeCm3 = volumeCm3 * Math.pow(scaleFactor, 3);
    logger.info(`   Scaled volume (${scalePercent}%): ${scaledVolumeCm3.toFixed(2)} cm³`);
    ...
    logger.info(`   Infill factor: ${infillFactor.toFixed(2)} (${shellFactor.toFixed(3)} shell + ${infillPercent}% infill)`);
    logger.info(`   Effective volume: ${effectiveVolumeCm3.toFixed(2)} cm³`);
    ...
    logger.info(`   Support volume: ${supportVolumeCm3.toFixed(2)} cm³ (${(supportFactor * 100).toFixed(1)}%)`);
    
    // Total volume including supports
    const totalVolumeCm3 = effectiveVolumeCm3 + supportVolumeCm3;
    logger.info(`   📦 Total material volume: ${totalVolumeCm3.toFixed(2)} cm³ (model + supports)`);
    
    // Get material density
    const density = MATERIAL_DENSITIES[material.toLowerCase()] || MATERIAL_DENSITIES.pla;
    logger.info(`   Material density: ${density} g/cm³`);
    
    // Calculate base weight (model + supports only)
    const baseWeightGrams = totalVolumeCm3 * density;
    logger.info(`⚖️  Base weight: ${baseWeightGrams.toFixed(1)}g (model + supports)`);
    ...
    logger.info(`⚖️  Final weight: ${finalWeightGrams}g (including purge/tower waste)`);
    logger.info(`   📊 Breakdown: ${baseWeightGrams.toFixed(1)}g × ${wasteMultiplier.toFixed(2)} = ${finalWeightGrams}g`);
    
    // Return base weight (model + supports) to match Bambu Studio
    const displayWeightGrams = Math.round(baseWeightGrams);
    logger.info(`📊 Display weight: ${displayWeightGrams}g (model + supports, matching Bambu Studio)`);
    
    // Calculate raw material cost
    const rawCost = Math.round(finalWeightGrams * pricePerGram);
    logger.info(`💰 Raw material cost: ₹${rawCost} (${pricePerGram}₹/g)`);
    ...
    logger.error('❌ Error calculating mesh weight:', error.message);
    throw error;
  }
}
